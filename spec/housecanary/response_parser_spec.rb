  ...
  describe '#perform' do
    context 'Success Response' do
      ...
      it 'returns body as symbolized hash' do
        ...
        expect(result.first.key?(:address_info)).to eq(true)
      end
    end
    ...
    describe 'Error Handling' do
      ...
      context 'if response code is OK but API :api_code different from 0' do
        ...
        it 'raises error with message according to code' do
          expect { subject.perform(error_response) }.to raise_error(Housecanary::Error::NoContent, a_string_ending_with("no content"))
        end
      end
    end
  end
